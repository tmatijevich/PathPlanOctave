...
function [solution, valid] = GetAccInTimeDiffWithRise(dt_tilde, dx, v_1, v_f, v_min, v_max, printResult = false)
	...
	if (v_min < 0.0) || (v_max <= v_min)
		printf("GetAccInTimeDiffWithRise call failed: Implausible velocity limits %.3f, %.3f\n", v_min, v_max); 
		valid = false; 
		return;
	
	% #2: Endpoint velocities within limits
	elseif (v_1 < v_min) || (v_1 > v_max) || (v_f < v_min) || (v_f > v_max)
		printf("GetAccInTimeDiffWithRise call failed: Endpoint velocities %.3f, %.3f exceed limits %.3f, %.3f\n", v0, v_f, v_min, v_max); 
		valid = false; 
		return;
	...
	if requires2ndOrderSolution
		...
		if !rootsValid
			printf("GetAccInTimeDiffWithRise call failed: Invalid roots for peak movement\n");
			valid = false;
			return;
			...
			if max(rootsSolution.r1, rootsSolution.r2) > 0.0 
				...
				printf("GetAccInTimeDiffWithRise call failed: Non-position 2nd order roots solution\n");
			end % Positive root?
		end % Valid roots?
