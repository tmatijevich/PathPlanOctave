...
function [soln, valid] = Kin_GetVelProfPoint(x0, timePoints, velPoints, n, t)
	...
	soln.a = accPoints(segment);
	soln.v = velPoints(segment) + accPoints(segment) * (t - timePoints(segment));
	soln.x = posPoints(segment) + velPoints(segment) * (t - timePoints(segment)) + 0.5 * accPoints(segment) * (t - timePoints(segment)) ^ 2;
